...
Runs the blessed policy in sim via ``leloop.scripts.collect_rollouts`` (a
...
from leloop.agents.base import Runner, subprocess_runner
...
from leloop.orchestrator.constitution import Constitution, ConstitutionError
...
class ImproveAgent:
    ...
    def build_command(self, checkpoint: CheckpointRecord, out_dir: Path, repo_id: str) -> list[str]:
        cmd = [
            sys.executable, "-m", "leloop.scripts.collect_rollouts",
