...
- 2026-07-09 · Residual RL is task-DEPENDENT, not a universal win — and the inner gate is what makes that safe. A 3rd go/no-go on task_id 8 (base 70% on the gate's held-out subset at n=5 — NOT the 40% its standard n=1 eval showed; the gate subset differs) gave composed 65% vs base 70%, **gain −0.05 = exactly 1 episode of 20 = noise → a WASH, gate SKIP HARVEST**. So the residual helped where there's headroom + fixable near-misses (task 4 +0.35, task 5 +0.20) but found nothing on task 8, and the §5.1 inner gate CORRECTLY refused to harvest it (validated a 3rd time — this time on a genuine non-improvement, not a collapse). Full-loop implication: train a residual per stuck task, harvest ONLY the ones where composed > base+0.05; non-improvements never pollute the mix. This selectivity is a feature — unconditional harvesting would be dangerous. (Caveat: the gate's 20 held-out states make ±0.05 the noise floor; report gains relative to that. Standard-eval per-task success ≠ the gate subset's base — pick target tasks by measuring base on the gate states, not the standard eval.)
