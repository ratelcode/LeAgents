...
**P2.1 — Single-task go/no-go (the feasibility gate). ← highest-value milestone.**
- Pick **one `libero_spatial` task the base already solves ~60–70%**. Train the residual (sparse reward, RLPD, α=0.5, ~100–150k steps) on `train_init_states`; measure composed vs base on `harvest_init_states`.
- **Success criterion:** composed ≥ base + `min_composed_gain` on held-out init states within the GPU budget, and wall-clock is acceptable (informs the per-task cost model). **If this fails, stop and reconsider** — everything downstream depends on it.
