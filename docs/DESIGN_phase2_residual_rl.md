...
**P2.2 — Target a stuck task (the real test). ← ✅ PASSED (2026-07-08).**
- Same machinery on `task_id 5` "on the ramekin" — the run's persistent laggard (base ~40% at n=5), which data growth never fixed. **Result: composed 60% vs base 40% on held-out states → gain +0.20, gate PASS.** So the residual helps NOT just an easy task (P2.1: 65%→100%) but the STUCK task too. Risk #1 did NOT bite here because the base ~40% *does attempt* the behavior, so the residual can nudge near-misses to success — the flywheel now has a mechanism to improve stuck tasks without new expert data. (Risk #1 would still bite a truly ~0% task where the base never attempts the grasp; task 5 at 40% is above that floor.)
- **Generalization + the gate's selectivity (a 3rd task).** `task_id 8` (base 70% on the held-out gate subset at n=5 — note: NOT its 40% standard-eval number; the gate init-states differ, so pick targets by measuring base on the gate states) gave composed 65% → **gain −0.05, gate SKIP**. That −0.05 is exactly 1 episode of the 20 held-out states = the noise floor: task 8 is a **wash**, not a degradation. **Takeaway: residual RL is task-dependent (helps tasks 4 & 5, no-op on task 8), and the inner gate (§5.1) correctly harvests only the wins.** This is the design working as intended — per-task residuals, harvest-gated, so non-improvements never pollute the mix. Unconditional harvesting would be unsafe; the gate is the safety.
